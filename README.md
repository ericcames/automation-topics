...
| 2026-06-17 | `customer-architect` | Terraform + AAP best practices — the provisioning/configuration boundary (Terraform builds/tears down the OS where APIs are good; AAP does everything after the OS and what Terraform can't), the "one owner of state" rule, immutable + golden-image flow, and the two integration paths (certified `hashicorp.terraform` for TFC/TFE vs `cloud.terraform` + a Terraform EE for CLI); TF flavor and target infra pushed into discovery | [sessions/2026-06-17_customer-architect_terraform-aap-best-practices](sessions/2026-06-17_customer-architect_terraform-aap-best-practices/) |
| 2026-06-18 | `customer-architect` | Build vs. buy — a lead engineer wants to **self-build an automation platform** (Ansible Core + AI-assisted code) instead of AAP, citing per-device cost, in a **life-safety** context. Reframes humanely: his **4-phase change framework** (pre-validate → push → post-validate → auto-rollback) **is already an AAP workflow + Event-Driven Ansible**; the **RHEL analogy** (Ansible Core → AAP :: Fedora/Stream → RHEL — he already chose "buy" for the OS); **bus-factor/supportability** as the exec argument; **TCO + cost-of-outage** vs. sticker price; **architect-not-maintainer**; per-audience tracks (engineer / exec / sales) | [sessions/2026-06-18_customer-architect_build-vs-buy-diy-ansible-vs-aap](sessions/2026-06-18_customer-architect_build-vs-buy-diy-ansible-vs-aap/) |
