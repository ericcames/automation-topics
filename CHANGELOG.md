...
- Same doc: **Reference configuration (template)** section — config-as-code for the one-time trust. Vault side is structurally complete Terraform (`vault_jwt_auth_backend` + `vault_policy` + `vault_jwt_auth_backend_role`) with a CLI equivalent; AAP side mirrors the proven `infra.aap_configuration` data-file pattern from `dc1.azure/aap_config` (`controller_credentials` consumed by the `dispatch`/`controller_credentials` role, attach via `controller_job_templates` `credentials:`). Notes that the collection resolves credential types by **name**, so it provisions an OIDC HashiCorp Vault credential the moment AAP 2.7 exposes that managed type — only the credential-type name and `inputs` field keys are 2.7-new; all other AAP-specific values are `<PLACEHOLDER>`s from AAP's OIDC discovery doc
...
### Changed
- [docs/aap-oidc-vault-identity.md](docs/aap-oidc-vault-identity.md) — **corrected against a live AAP 2.7 build (2026-06-11).** The build exposes only the two classic HashiCorp Vault credential types (`HashiCorp Vault Secret Lookup`, `HashiCorp Vault Signed SSH`) with Token/AppRole/TLS/Kubernetes/userpass auth — there is **no `(OIDC)` variant and no JWT/OIDC auth field yet**. Renamed the invented `…(OIDC)` types to the real ones, reframed the AAP-issued-JWT→Vault flow as the **announced direction** (confirmed conceptually by the Red Hat/HashiCorp blog) rather than a configurable feature, fixed the config-as-code `credential_type`, and added a **Verified against a live AAP 2.7 build** section with two screenshots ([docs/images/aap27-vault-credential-types.png](docs/images/aap27-vault-credential-types.png), [aap27-vault-secret-lookup-fields.png](docs/images/aap27-vault-secret-lookup-fields.png)). Vault-side Terraform is unaffected (standard JWT auth method)
- Same doc: added **"The client side already supports JWT — the gap is narrow"** — the certified `hashicorp.vault` collection (formerly `community.hashi_vault`) already ships a `jwt` auth method (`auth_login` with `auth_method: jwt`), so the Vault auth method **and** the Ansible client are both available today; the only missing piece is AAP surfacing each job's minted JWT to the EE. Includes a play snippet and a three-part status table, plus a `hashicorp.vault` entry in References
