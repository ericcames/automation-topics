...
### Added
- Session [2026-06-17_customer-architect_terraform-aap-best-practices](sessions/2026-06-17_customer-architect_terraform-aap-best-practices/) — **Terraform + AAP best practices** for a customer + account-team working session. Built around one boundary: **Terraform provisions/deprovisions infrastructure + the OS where high-quality provider APIs exist; AAP does everything after the OS and covers what Terraform can't reach** — plus the non-negotiable rule that **one system owns the state file (Terraform); AAP reads outputs read-only and never writes state**. Covers the immutable + golden-image flow (**build images with AAP → Terraform deploys → AAP configures/redeploys**), the two integration directions (**AAP orchestrates Terraform** vs **pipeline hands off to AAP**), and both tooling paths: the Red Hat-certified **`hashicorp.terraform`** collection for **Terraform Cloud / Enterprise** (`workspace`/`project`/`configuration_version`/`run`/`output`/`view_plan`) vs **`cloud.terraform`** + a pinned **Terraform execution environment** for open-source `terraform` CLI. Terraform flavor (TFC/TFE/CLI), state location/locking, and target infra (cloud/on-prem/hybrid) are pushed into discovery. Architect-led with SSE-depth go-deep; references the certified collection, its upstream repo, and a custom-EE POC
- Session [2026-06-16_customer-sse_rhel-patching-test-promote-rollback](sessions/2026-06-16_customer-sse_rhel-patching-test-promote-rollback/) — RHEL patching modernization for an AAP-on-OpenShift customer (2.6 test / 2.5 prod) who wants to **test patches before production**. Frames the target state as a **Satellite Content View + lifecycle-environment** promote pipeline and answers every operational question they raised: rollback (CV revert / LVM+`boom` snapshot / `dnf history undo` for routine patching vs **Leapp snapshot-before / restore-on-failure** for RHEL 7→8 major upgrades), staggered **ring** rollout (one job template + many schedules + surveys), Satellite **dynamic inventory** group-picking, a **ServiceNow** change-gate → patch tasks → CMDB-update → close workflow, and **Insights "Remediate with Ansible"** push-button. SUSE (`zypper` via AAP; Satellite doesn't manage SUSE content), RHEL-on-Power (ppc64le, a footnote), and Qualys (clarify-then-position vs Insights) noted but kept out of the core design
